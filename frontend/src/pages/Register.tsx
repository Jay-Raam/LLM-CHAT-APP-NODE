...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // register then login via auth helper so cookies are set
      await apiRegister({ name, email, password });
      const loginResponse = await apiLogin({ email, password });
      dispatch(setAuth({ user: loginResponse.user || null, token: loginResponse.token || null }));
      navigate('/');
    } catch (err: any) {
      dispatch(setError(err.response?.data?.error || err.message || 'Registration failed. Please try again.'));
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center p-4 bg-[#0A0A0A]">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400 mt-2">Join Nexus AI and start chatting</p>
        </div>
